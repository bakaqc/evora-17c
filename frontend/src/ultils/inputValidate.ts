...
const isValidVoucher = (voucher: string): boolean => {
	const voucherValue = parseFloat(voucher);
	return !isNaN(voucherValue) && voucherValue > 0 && voucherValue <= 100;
};

export const validateVoucher = (voucher: string): boolean => {
	const trimmedVoucher = trimInput(voucher);
	return isValidVoucher(trimmedVoucher);
};
